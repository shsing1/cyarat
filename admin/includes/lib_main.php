 ...
 * ============================================================================
 * Author: shsing1
 * Id: lib_main.php 2009-10-24 11:00:00
*/
...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['chh']->table('admin_log') . ' (log_time, id, log_info, ip_address) ' .
 ...
 *
 * @access  public
 * @param   integer $id        管理員編號
 * @param   string  $username       管理員姓名
 * @param   string  $action_list    權限列表
 * @param   string  $last_time      最後登錄時間
 * @return  void
 */
function set_admin_session($id, $cat_id, $username, $action_list, $last_time)
{
    $_SESSION['admin_id']    	= $id;
	$_SESSION['admin_cat_id']   = $cat_id;
...
function insert_config($parent, $code, $value)
{
    ...
    $sql = 'SELECT id FROM ' . $chh->table('config') . " WHERE code = '$parent' AND type = 1";
    $cat_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $chh->table('config') . ' (cat_id, code, value) ' .
            "VALUES('$cat_id', '$code', '$value')";
    $db->query($sql);
}
...
{
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['chh']->table('bonus_type') .
           ' WHERE send_type = 3';
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['chh']->table('user_rank');
    if ($is_special)
    {
 ...
 * 按等級取得用戶列表（用於生成下拉列表）
 *
 * @return  array       分類數組id => name
 */
function get_user_rank($rankid, $where)
{
    $user_list = array();
    $sql = 'SELECT id, name FROM ' . $GLOBALS['chh']->table('users') . $where.
           ' ORDER BY id DESC';
    $res = $GLOBALS['db']->query($sql);
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['chh']->table('ad_position');
    $res = $GLOBALS['db']->query($sql);
...
/*	$str = '<script type="text/javascript" src="'.$chh->url().'includes/ckeditor/ckeditor.js"></script>';
	$str .= '<script type="text/javascript" src="'.$chh->url().'includes/ckeditor/ckfinder/ckfinder.js"></script>';
	$str .='<textarea class="ckeditor" id="'.$input_name.'" name="'.$input_name.'" >'.$input_value.'</textarea>';
	$str .= '<script type="text/javascript">'.
			"	CKFinder.SetupCKEditor( CKEDITOR, '../includes/ckeditor/ckfinder/' );".
			'</script>';
    $smarty->assign('FCKeditor_'.$input_name, $str);
*/	
}
...
function get_goods_list($filter)
{
    ...
    $sql = 'SELECT goods_id, goods_name, shop_price '.
           'FROM ' . $GLOBALS['chh']->table('goods') . ' AS g ' . $where .
           'LIMIT 50';
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title '.
           'FROM ' .$GLOBALS['chh']->table('article'). ' AS a, ' .$GLOBALS['chh']->table('article_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function area_list($region_id)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['chh']->table('region').
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['chh']->table('goods_type') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' .$GLOBALS['chh']->table('payment'). ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['chh']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['chh']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
function get_filter($param_str = '')
{
    ...
    if (isset($_GET['uselastfilter']) && isset($_COOKIE['CHH']['lastfilterfile'])
        && $_COOKIE['CHH']['lastfilterfile'] == sprintf('%X', crc32($filterfile)))
    {
        return array(
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['chh']->table('category').
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['chh']->table('brand').
...
function admin_info()
{
    $sql = "SELECT * FROM ". $GLOBALS['chh']->table('admin_user')."
