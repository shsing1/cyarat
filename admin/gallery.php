 ...
 * ============================================================================
 * Author: shsing1
 * Id: gallery.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_gallery_img.php');
$data_img = new cls_gallery_img($db, $chh->table("gallery_img") );
...
$ur_here = $sys_menu->get_ur_here(12);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'gallery.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('gallery_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result($smarty->fetch('gallery_list.htm'), '',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'gallery.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('gallery_info.htm');
	...
	case 'insert':
		...
		if(!$data->add($field)){
			make_json_error($_LANG['data_add_failed']);
																...
																'op'=>'add',
																'url'=>'gallery.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'gallery.php?act=list'));
		
		/* 創建 html editor */
		create_html_editor('desc', $data_info['desc']);
	
		/* 取得資料gallery data */
		$gallery = $data_img->get_list($id);
	
		foreach($gallery['list'] as $k=>$v){
			$v['thumb']	= '../../'.$v['thumb'];
			$v['img']	= '../../'.$v['img'];
			$v['original_img']	= '../../'.$v['original_img'];
			$gallery['list'][$k] = $v;
		...
		$smarty->assign('img_list', $gallery['list']);
...
/*------------------------------------------------------ */
	case 'remove':
		...
		}make_json_result('', $_LANG['data_del_succed'], array('url'=>'gallery.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'gallery.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'gallery.php?'.get_last_filter_url() ));

		break;
/*------------------------------------------------------ */
//-- 上傳圖片管理的圖片處理
/*------------------------------------------------------ */
	case 'upload_gallery_img':
		$id 		= isset($_REQUEST['id'])		? intval($_REQUEST['id'])		: 0;
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'gallery.php?act=edit&id='.$id ) );
