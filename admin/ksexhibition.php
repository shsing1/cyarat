 ...
 * ============================================================================
 * Author: shsing1
 * Id: ksexhibition.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_ksexhibition_cat.php');
$cat = new cls_ksexhibition_cat($db, $chh->table("ksexhibition_cat") );

require_once(ROOT_PATH . '/includes/cls_ksexhibition.php');
$data = new cls_ksexhibition($db, $chh->table("ksexhibition"), $cat);
...
$ur_here = $sys_menu->get_ur_here(105);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'ksexhibition.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('ksexhibition_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('ksexhibition_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'ksexhibition.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('ksexhibition_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		make_json_result('', $_LANG['data_add_succed'], array('url'=>'ksexhibition.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'ksexhibition.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':	
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'ksexhibition.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'ksexhibition.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'ksexhibition.php?'.get_last_filter_url() ) );
