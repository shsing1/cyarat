 ...
 * ============================================================================
 * Author: shsing1
 * Id: mlreservation.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_mlreservation_cat.php');
$cat = new cls_mlreservation_cat($db, $chh->table("mlreservation_cat") );

require_once(ROOT_PATH . '/includes/cls_mlreservation.php');
$data = new cls_mlreservation($db, $chh->table("mlreservation"), $cat);
...
$ur_here = $sys_menu->get_ur_here(89);
...
/*------------------------------------------------------ */
	case 'list':
		...
		$smarty->assign('action_link',  array('href' => 'mlreservation.php?act=add', 'text' => $_LANG['data_add']));
		}
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('mlreservation_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('mlreservation_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'mlreservation.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('mlreservation_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		$content = $smarty->fetch('mlreservation_add_mail_tpl.htm');
		...
		make_json_result('', $_LANG['data_add_succed'], array('url'=>'mlreservation.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'mlreservation.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':	
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'mlreservation.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'mlreservation.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'mlreservation.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'reply':	
		...
		assign_query_info();
		$smarty->assign('ur_here',     $ur_here . ' - '.$data_info['name'] );
		$smarty->display('mlreservation_reply_info.htm');
...
/*------------------------------------------------------ */
	case 'reply_insert':	
		$field['mlreservation_id']		= !isset($_REQUEST['id'])			?	0	: intval($_REQUEST['id']);
		$field['name']     			= !isset($_POST['name'])     		?	''	: trim($_POST['name']);
		$field['email']				= !isset($_POST['email'])   		?	''	: trim($_POST['email']);
		$field['phone']				= !isset($_POST['phone'])			?	''	: trim($_POST['phone']);
		$field['sort']   			= !isset($_POST['sort'])     		?	0	: intval($_POST['sort']);
		...
		$smarty->assign('reply_info',	$reply_info);
		$content = $smarty->fetch('mlreservation_reply_mail_tpl.htm');
		if (!send_mail($data_info['name'], $data_info['email'], $_LANG['reply_mail_title'], $content, 1))
		{
			make_json_error(join("\n", $err->_message));
		}
		
		make_json_result('', $_LANG['replay_add_succe'], array('url'=>'mlreservation.php?'.get_last_filter_url() ) );
