 ...
 * ============================================================================
 * Author: shsing1
 * Id: ksnews.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_ksnews_cat.php');
$cat = new cls_ksnews_cat($db, $chh->table("ksnews_cat") );

require_once(ROOT_PATH . '/includes/cls_ksnews.php');
$data = new cls_ksnews($db, $chh->table("ksnews"), $cat);
...
$ur_here = $sys_menu->get_ur_here(114);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'ksnews.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('ksnews_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result($smarty->fetch('ksnews_list.htm'), '',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'ksnews.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('ksnews_info.htm');
...
//		make_json_result('', $_LANG['data_add_succed'], array('url'=>'ksnews.php?'.get_last_filter_url() ));
		make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'img', 
																'id'=>$data->get_insert_id(),
																'op'=>'add',
																'url'=>'ksnews.php?'.get_last_filter_url() ));
		break;
/*------------------------------------------------------ */
//-- 編輯資料
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'ksnews.php?act=list'));
...
//		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'ksnews.php?'.get_last_filter_url() ));
		$id = $field['id'];
...
/*------------------------------------------------------ */
	case 'remove':
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'ksnews.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'ksnews.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'ksnews.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'ksnews.php?act=edit&id='.$id ) );
