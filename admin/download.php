 ...
 * ============================================================================
 * Author: shsing1
 * Id: download.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_download_cat.php');
$cat = new cls_download_cat($db, $chh->table("download_cat") );

require_once(ROOT_PATH . '/includes/cls_download.php');
$data = new cls_download($db, $chh->table("download"), $cat);
...
$ur_here = $sys_menu->get_ur_here(22);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'download.php?act=add', 'text' => $_LANG['data_add']));
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result($smarty->fetch('download_list.htm'), '',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'download.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('download_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':
		...
		}make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'download_file', 
																'id'=>$data->get_insert_id(),
																'op'=>'add',
																'url'=>'download.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'download.php?act=list'));
		...
		/* 取得資料download file  */
		$files = $data_file->get_list($id);	
		$smarty->assign('file_list', $files['list']);
...
/*------------------------------------------------------ */
	case 'update':
		...
		/* 更新下載檔案簡介及排序 */
		$field['old_file_brief']    = !empty($_POST['old_file_brief'])     	? $_POST['old_file_brief']     				: array();
		$field['old_file_sort']		= !empty($_POST['old_file_sort'])     	? $_POST['old_file_sort']     				: array();	
		/* 取得原有的簡介及排序 */
		$old_brief = array();
		$old_sort = array();
		foreach($field['old_file_brief'] as $k=>$v){
			$info = $data_file->get_info($k);
			$old_brief[$k] = $info['brief'];
			$old_sort[$k]	 = $info['sort'];
		}
		
		/* 將有修改的資料挑選出來 */
		$field['old_file_brief']	= array_diff_assoc($field['old_file_brief'], $old_brief);
		$field['old_file_sort']		= array_diff_assoc($field['old_file_sort'], $old_sort);
		...
		foreach($field['old_file_brief'] as $k=>$v){
			...
			$t['brief'] = $v;
			if(!$data_file->upd($t)){
				make_json_error($_LANG['download_file_upd_failed']);
			}
		}
		
		/* 更新排序*/
		foreach($field['old_file_sort'] as $k=>$v){
			$t['id'] 	= $k;
			$t['sort'] 	= $field['old_file_sort'][$k];
			if(!$data_file->upd($t)){
				make_json_error($_LANG['download_file_upd_failed']);
			}
		}		
		
		make_json_result('', $_LANG['data_upd_succed'], array(	'upload'=>'download_file', 
																'id'=>$id,
																'op'=>'upd',
																'url'=>'download.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'remove':
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'download.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'download.php?'.get_last_filter_url() ) );

		break;
/*------------------------------------------------------ */
//-- 上傳下載檔案的處理
/*------------------------------------------------------ */
	case 'upload_download_file':
		$id 		= !isset($_REQUEST['id'])			? 0		: intval($_REQUEST['id']);
		$file_brief	= !isset($_REQUEST['file_brief'])	? ''	: trim($_REQUEST['file_brief']);
		$file_sort	= !isset($_REQUEST['file_sort'])	? 0		: intval($_REQUEST['file_sort']);
		$op 		= !isset($_REQUEST['op'])			? 'add'	: trim($_REQUEST['op']);
		
		$field['download_id'] 	= $id;
		$field['brief'] 		= $file_brief;
		$field['sort'] 			= $file_sort;
	
		if(!$data_file->add($field, $_FILES)){
			make_json_error($_LANG['upload_file_failed']);
		}
		
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'download.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_download_file':
		...
		make_json_result('', $_LANG['file_del_succed']);
