 ...
 * ============================================================================
 * Author: shsing1
 * Id: indexbg.php 2009-11-27 12:00:00
*/
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'indexbg.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('indexbg_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('indexbg_list.htm'),
							'',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'indexbg.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('indexbg_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':
		...
		$field['main_title']		= !isset($_POST['main_title'])   	?	''	: trim($_POST['main_title']);
		$field['sub_title']	= 		!isset($_POST['sub_title'])			?	''	: trim($_POST['sub_title']);
		...
		// make_json_result('', $_LANG['data_add_succed'], array('url'=>'indexbg.php?'.get_last_filter_url() ) );
		...
		make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'img',
																...
																'op'=>'add',
																'url'=>'indexbg.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'indexbg.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':
		...
		//make_json_result('', $_LANG['data_upd_succed'], array('url'=>'indexbg.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'indexbg.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'indexbg.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'indexbg.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'indexbg.php?act=edit&id='.$id ) );
