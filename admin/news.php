 ...
 * ============================================================================
 * Author: shsing1
 * Id: news.php 2009-11-27 12:00:00
*/
...
$ur_here = $sys_menu->get_ur_here(9);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'news.php?act=add', 'text' => $_LANG['data_add']));
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'news.php?act=list', 'text' => $_LANG['data_list']));
...
//		make_json_result('', $_LANG['data_add_succed'], array('url'=>'news.php?'.get_last_filter_url() ));
		make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'img', 
																'id'=>$data->get_insert_id(),
																'op'=>'add',
																'url'=>'news.php?'.get_last_filter_url() ));
		break;
/*------------------------------------------------------ */
//-- 編輯資料
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'news.php?act=list'));
...
//		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'news.php?'.get_last_filter_url() ));
		$id = $field['id'];
...
/*------------------------------------------------------ */
	case 'remove':
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'news.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'news.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'news.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'news.php?act=edit&id='.$id ) );
