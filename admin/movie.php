 ...
 * ============================================================================
 * Author: shsing1
 * Id: movie.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_movie_cat.php');
$cat = new cls_movie_cat($db, $chh->table("movie_cat") );

require_once(ROOT_PATH . '/includes/cls_movie.php');
$data = new cls_movie($db, $chh->table("movie"), $cat);
...
$ur_here = $sys_menu->get_ur_here(52);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'movie.php?act=add', 'text' => $_LANG['data_add']));
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('movie_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'movie.php?act=list', 'text' => $_LANG['data_list']));
...
//		make_json_result('', $_LANG['data_add_succed'], array('url'=>'movie.php?'.get_last_filter_url() ) );
		
		make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'img', 
																'id'=>$data->get_insert_id(),
																'op'=>'add',
																'url'=>'movie.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'movie.php?act=list'));
...
//		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'movie.php?'.get_last_filter_url() ) );
		$id = $field['id'];
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'movie.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'movie.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'movie.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'movie.php?act=edit&id='.$id ) );
