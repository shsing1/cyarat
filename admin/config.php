...
$ur_here = $sys_menu->get_ur_here(18);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'config.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('config_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result($smarty->fetch('config_list.htm'), '',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'config.php?act=list', 'text' => $_LANG['data_list']));
		
		$cat_select = $cat->get_list_option();
		$smarty->assign('cat_select',   $cat_select);
		
		$smarty->assign('selected',   	0); //類型預設值
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('config_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':
		...
		$field['code']			= !isset($_POST['code'])     	?	''	:	trim($_POST['code']);
		$field['type']			= !isset($_POST['type'])    	?	''	:	trim($_POST['type']);
		$field['store_range']	= !isset($_POST['store_range']) ?	''	:	trim($_POST['store_range']);
		$field['value']			= !isset($_POST['value'])    	?	''	:	trim($_POST['value']);
		...
		}make_json_result('', $_LANG['data_add_succed'], array('url'=>'config.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'config.php?act=list'));
		...
		$smarty->assign('selected',   	$data_info['type']); 			//類型預設值
...
/*------------------------------------------------------ */
	case 'update':
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'config.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'config.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'config.php?'.get_last_filter_url() ) );
