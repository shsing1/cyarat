 ...
 * ============================================================================
 * Author: shsing1
 * Id: art.php 2009-11-27 12:00:00
*/
...
$ur_here = $sys_menu->get_ur_here(54);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'art.php?act=add', 'text' => $_LANG['data_add']));
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result($smarty->fetch('art_list.htm'), '',
...
/*------------------------------------------------------ */
	case 'add':
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'art.php?act=list', 'text' => $_LANG['data_list']));
...
/*------------------------------------------------------ */
	case 'insert':
		...
		$field['author_title']     	= isset($_POST['author_title'])     	? trim($_POST['author_title'])     	: '';
		$field['author_desc']     	= isset($_POST['author_desc'])     		? trim($_POST['author_desc'])     	: '';
		$field['works_title']     	= isset($_POST['works_title'])     		? trim($_POST['works_title'])     	: '';
		$field['works_desc']     	= isset($_POST['works_desc'])     		? trim($_POST['works_desc'])     	: '';
		...
		if(!$data->add($field)){
			make_json_error($_LANG['data_add_failed']);
																...
																'op'=>'add',
																'url'=>'art.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'art.php?act=list'));

		/* 創建 html editor */
		create_html_editor('desc', $data_info['desc']);

		/* 取得資料art data */
		$art = $data_img->get_img_list($id);

		foreach($art['list'] as $k=>$v){
			$v['thumb']	= '../../'.$v['thumb'];
			$v['img']	= '../../'.$v['img'];
			$v['original_img']	= '../../'.$v['original_img'];
			$art['list'][$k] = $v;
		...
		$smarty->assign('img_list', $art['list']);
...
/*------------------------------------------------------ */
	case 'remove':
		...
		}make_json_result('', $_LANG['data_del_succed'], array('url'=>'art.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'batch':
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'art.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'art.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'art.php?act=edit&id='.$id ) );
