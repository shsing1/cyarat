 ...
 * ============================================================================
 * Author: shsing1
 * Id: planning.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_planning_cat.php');
$cat = new cls_planning_cat($db, $chh->table("planning_cat") );

require_once(ROOT_PATH . '/includes/cls_planning.php');
$data = new cls_planning($db, $chh->table("planning"), $cat);
...
$ur_here = $sys_menu->get_ur_here(94);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'planning.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('planning_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('planning_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'planning.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('planning_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		make_json_result('', $_LANG['data_add_succed'], array('url'=>'planning.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'planning.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':	
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'planning.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'planning.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'planning.php?'.get_last_filter_url() ) );
