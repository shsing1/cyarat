 ...
 * ============================================================================
 * Author: shsing1
 * Id: mlsponsors.php 2009-11-27 12:00:00
*/
...
$ur_here = $sys_menu->get_ur_here(80);
...
/*------------------------------------------------------ */
	case 'list':
		...
		foreach($data_list['list'] as $k=>$v){
			$cat_info = $cat->get_info($v['cat_id']);
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'mlsponsors.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('mlsponsors_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('mlsponsors_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'mlsponsors.php?act=list', 'text' => $_LANG['data_list']));
		...
		$smarty->assign('data_info',     array('is_show' => 1, 'is_target' => 1));
	
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('mlsponsors_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		$field['is_target']     	= !isset($_POST['is_target'])      	?	1	: intval($_POST['is_target']);
...
//		make_json_result('', $_LANG['data_add_succed'], array('url'=>'mlsponsors.php?'.get_last_filter_url() ) );
		
		make_json_result('', $_LANG['data_add_succed'], array(	'upload'=>'img', 
																'id'=>$data->get_insert_id(),
																'op'=>'add',
																'url'=>'mlsponsors.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'mlsponsors.php?act=list'));
...
//		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'mlsponsors.php?'.get_last_filter_url() ) );
		$id = $field['id'];
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'mlsponsors.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'mlsponsors.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'upload_img':
		...
		make_json_result('', $_LANG['data_'.$op.'_succed'], array('url'=>'mlsponsors.php?'.get_last_filter_url() ));
...
/*------------------------------------------------------ */
	case 'drop_img':
		...
		make_json_result('', $_LANG['img_del_succed'], array('url'=>'mlsponsors.php?act=edit&id='.$id ) );
