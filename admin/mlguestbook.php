 ...
 * ============================================================================
 * Author: shsing1
 * Id: mlguestbook.php 2009-11-27 12:00:00
*/
...
require_once(ROOT_PATH . '/includes/cls_mlguestbook_cat.php');
$cat = new cls_mlguestbook_cat($db, $chh->table("mlguestbook_cat") );

require_once(ROOT_PATH . '/includes/cls_mlguestbook.php');
$data = new cls_mlguestbook($db, $chh->table("mlguestbook"), $cat );

require_once(ROOT_PATH . '/includes/cls_mlguestbook_reply.php');
$data_reply = new cls_mlguestbook_reply($db, $chh->table("mlguestbook_reply") );
...
$ur_here = $sys_menu->get_ur_here(83);
...
/*------------------------------------------------------ */
	case 'list':
		...
		$smarty->assign('action_link',  array('href' => 'mlguestbook.php?act=add', 'text' => $_LANG['data_add']));
		}
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('mlguestbook_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('mlguestbook_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'mlguestbook.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('mlguestbook_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		$content = $smarty->fetch('mlguestbook_add_mail_tpl.htm');
		...
		make_json_result('', $_LANG['data_add_succed'], array('url'=>'mlguestbook.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'mlguestbook.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':	
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'mlguestbook.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'mlguestbook.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'mlguestbook.php?'.get_last_filter_url() ) );
...
//		$reply_info = $data_admin->get_info($_SESSION['admin_id']);
		$reply_info['name'] = $_CFG['guestbook_name'];
		$reply_info['email'] = $_CFG['guestbook_email'];
		...
		$smarty->display('mlguestbook_reply_info.htm');
...
/*------------------------------------------------------ */
	case 'reply_insert':	
		$field['mlguestbook_id']		= !isset($_REQUEST['id'])			?	0	: intval($_REQUEST['id']);
		$field['name']     			= !isset($_POST['name'])     		?	''	: trim($_POST['name']);
		$field['email']				= !isset($_POST['email'])   		?	''	: trim($_POST['email']);
		$field['phone']				= !isset($_POST['phone'])			?	''	: trim($_POST['phone']);
		$field['is_show']     		= !isset($_POST['is_show'])     	? 	1	: intval($_POST['is_show']);
...
//		}
		...
		$smarty->assign('reply_info',	$reply_info);
		$content = $smarty->fetch('mlguestbook_reply_mail_tpl.htm');
		if (!send_mail($data_info['name'], $data_info['email'], $_LANG['reply_mail_title'], $content, 1))
		{
//			make_json_error(join("\n", $err->_message));
		}
		
		make_json_result('', $_LANG['replay_add_succe'], array('url'=>'mlguestbook.php?act=reply&id='.$id) );
