 ...
 * ============================================================================
 * Author: shsing1
 * Id: epaper_user.php 2009-11-27 12:00:00
*/
...
$cat = new cls_epaper_user_cat($db, $chh->table("epaper_user_cat") );

require_once(ROOT_PATH . '/includes/cls_epaper_user.php');
$data = new cls_epaper_user($db, $chh->table("epaper_user"), $cat);
...
$ur_here = $sys_menu->get_ur_here(40);
...
/*------------------------------------------------------ */
	case 'list':
		...
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'epaper_user.php?act=add', 'text' => $_LANG['data_add']));
		...
		/* 列表頁面 */
		assign_query_info();
		$smarty->display('epaper_user_list.htm');
...
/*------------------------------------------------------ */
	case 'query':
		...
		make_json_result(	$smarty->fetch('epaper_user_list.htm'), 
							'',
...
/*------------------------------------------------------ */
	case 'add':	
		/* 模板賦值 */
		$smarty->assign('action_link',  array('href' => 'epaper_user.php?act=list', 'text' => $_LANG['data_list']));
		...
		/* 顯示頁面 */
		assign_query_info();
		$smarty->display('epaper_user_info.htm');
...
/*------------------------------------------------------ */
	case 'insert':	
		...
		/* email是否已經存在 */
		if($data->is_duplicate(array('email'=>$field['email']))){
			make_json_error($_LANG['email_is_duplicate']);
		}
		...
		make_json_result('', $_LANG['data_add_succed'], array('url'=>'epaper_user.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'edit':	
		...
		$smarty->assign('action_link', array('text' => $_LANG['data_list'], 'href' => 'epaper_user.php?act=list'));
...
/*------------------------------------------------------ */
	case 'update':	
		...
		if($info['email'] != $field['email'] && $data->is_duplicate(array('email'=>$field['email']))){
			make_json_error($_LANG['email_is_duplicate']);
		}
		...
		make_json_result('', $_LANG['data_upd_succed'], array('url'=>'epaper_user.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'remove':	
		...
		make_json_result('', $_LANG['data_del_succed'], array('url'=>'epaper_user.php?'.get_last_filter_url() ) );
...
/*------------------------------------------------------ */
	case 'batch':	
		...
		make_json_result('', $_LANG['data_batch_succed'], array('url'=>'epaper_user.php?'.get_last_filter_url() ) );
