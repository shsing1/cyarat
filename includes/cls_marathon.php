<?php
	...
	class cls_marathon extends cls_data {
		var $upload_folder	= 'marathon';
		var $img_width		= 190;
		var $img_height		= 148;
//		var $img2_width		= 314;
//		var $img2_height	= 107;
...
//			$feild['img2'] = '';
		...
		function set_page_size($size=15){
			...
			setcookie('marathon_page_size', intval($size), $t);
		}

		// 取得單頁顯示數
		function get_page_size(){
			$size = 15;
			if(isset($_COOKIE['marathon_page_size'])){
				$size = $_COOKIE['marathon_page_size'];
			}
		...
		function add_hit($id=0){
			if(!isset($_COOKIE['marathon_hit_'.$id])){
				$info =  $this->get_info($id);
				$feild = array();
				$feild['id'] = $info['id'];
				...
				$this->upd($feild);
				$t = strtotime("+1 day -1 seconds", strtotime(date('Y-m-d')));
				setcookie('marathon_hit_'.$id, 1, $t);
			}
		}
