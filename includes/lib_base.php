 ...
 * ============================================================================
 * Author: shsing1
 * Id: lib_base.php 2009-10-24 11:00:00
*/
...
function sub_str($str, $length = 0, $append = true)
{
    ...
    {
        $newstr = mb_substr($str, 0, $length, CHH_CHARSET);
    }
    elseif (function_exists('iconv_substr'))
    {
        $newstr = iconv_substr($str, 0, $length, CHH_CHARSET);
    }
    else
    {
...
/**
 * 將對像成員變量或者數組的特殊字符進行轉義
 *
 * @access   public
 * @param    mix        $obj      對像或者數組
 * @author   Xuan Yan
 *
 * @return   mix                  對像或者數組
 */
 ...
 * @return  void
 **/
function chh_header($string, $replace = true, $http_response_code = 0)
{
...
function chh_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
{
    if (CHH_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return chh_iconv('utf-8', CHH_CHARSET, $str);
        }
        elseif (is_array($str))
        {
 ...
 * @return
 */
//function authcode($string, $operation = 'DECODE', $key = 'piBFhMt3Mq5UKHgQrCb9', $expiry = 0) {
//	$ckey_length = 4;
//	$key = md5($key ? $key : '');
...
//function authcode($string, $operation = 'DECODE', $key = 'piBFhMt3Mq5UKHgQrCb9') {
//	if($operation == 'DECODE') {
...
function authcode($string, $operation = 'DECODE', $is_encode=false) {
	if($is_encode){
		$key = md5(microtime());
