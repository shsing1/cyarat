...
//        }
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
        ...
        {
            $dir = ROOT_PATH . $this->images_dir . '/' . $path.'/';
        }
		...
		if($this->cut_type == 1){
			$thumb->adaptiveResize($thumb_width, $thumb_height)->save( $dir . $filename);
		}else{
			$thumb->resize($thumb_width, $thumb_height)->save( $dir . $filename);
		}
     ...
     * @return  mix         如果成功返回縮略圖的路徑，失敗則返回false
     */
    function make_thumb_bk($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
         $gd = $this->gd_version(); //獲取 GD 版本。0 表示沒有 GD 庫，1 表示 GD 1.x，2 表示 GD 2.x
         if ($gd == 0)
         {
     ...
     * @return      mix         如果成功則返回文件路徑，否則返回false
     */
