<?php
	...
	class cls_art_img extends cls_data {

		var $foreign_name	= 'art_id';
		...
		function upload_img($file, $id = 0){
			...
			if (isset($file['art_img']['error'])) // php 4.2 版本才支持 error
			{
				// 最大上傳文件大小
			...
			// 如果上傳圖片，相應處理
			if ($file['art_img']['tmp_name'] != '' && $file['art_img']['tmp_name'] != 'none')
			{
				if ($id > 0)
				{
					/* 刪除原來的圖片文件 */
				...
				$original_img   = $image->upload_image($file['art_img'], $this->upload_folder); // 原始圖片
				...
				/* 依副檔名設定檔案形態 */
				$suffix = get_file_suffix($file['art_img']['name']);
				switch($suffix){
					case 'gif':
						$file['art_img']['type'] = 'image/gif';
						break;
					case 'png':
						$file['art_img']['type'] = 'image/png';
						break;
					default:
						$file['art_img']['type'] = 'image/jpeg';
						break;
				}

				// 如果系統支持GD，縮放圖片，且給圖片和相冊圖片加水印
				if ($image->gd_version() > 0 && $image->check_img_function($file['art_img']['type']))
				{
					// 如果設置大小不為0，縮放圖片
