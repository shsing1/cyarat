<?php
	...
	class cls_ksnews extends cls_data {
		var $upload_folder	= 'ksnews';	
		var $img_width		= 190;
		var $img_height		= 148;
//		var $img2_width		= 314;
//		var $img2_height	= 107;
...
//			$feild['img2'] = '';
		...
		function set_page_size($size=15){
			...
			setcookie('ksnews_page_size', intval($size), $t);			
		}
		
		// 取得單頁顯示數
		function get_page_size(){
			$size = 15;
			if(isset($_COOKIE['ksnews_page_size'])){
				$size = $_COOKIE['ksnews_page_size'];
			}
		...
		function add_hit($id=0){
			if(!isset($_COOKIE['ksnews_hit_'.$id])){
				$info =  $this->get_info($id);
				$feild = array();
				$feild['id'] = $info['id'];
