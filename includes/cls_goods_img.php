<?php
	...
	class cls_goods_img extends cls_data {
		
		var $foreign_name	= 'goods_id';
		var $upload_folder	= 'goods';	
		var $img_width		= 300;
		var $img_height		= 300;		
		var $thumb_width	= 50;
		var $thumb_height	= 50;
		...
		function upload_img($file, $id = 0){
			...
			if (isset($file['goods_img']['error'])) // php 4.2 版本才支持 error
			{
				// 最大上傳文件大小
				...
				// 圖片
				if ($file['goods_img']['error'] == 1)
				{
					make_json_error(sprintf($_LANG['img_too_big'], $php_maxsize));
				}
				elseif ($file['goods_img']['error'] == 2)
				{
			...
			// 如果上傳圖片，相應處理
			if ($file['goods_img']['tmp_name'] != '' && $file['goods_img']['tmp_name'] != 'none')
			{
				if ($id > 0)
				{
					/* 刪除原來的圖片文件 */
				...
				$original_img   = $image->upload_image($file['goods_img'], $this->upload_folder); // 原始圖片
				...
				/* 依副檔名設定檔案形態 */			
				$suffix = get_file_suffix($file['goods_img']['name']);
				switch($suffix){
					case 'gif':
						$file['goods_img']['type'] = 'image/gif';
						break;
					case 'png':
						$file['goods_img']['type'] = 'image/png';
						break;
					default:
						$file['goods_img']['type'] = 'image/jpeg';
						break;
				}
				
				// 如果系統支持GD，縮放圖片，且給圖片和相冊圖片加水印			
				if ($image->gd_version() > 0 && $image->check_img_function($file['goods_img']['type']))
				{
					// 如果設置大小不為0，縮放圖片
