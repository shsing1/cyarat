<?php
	...
	class cls_ksguestbook extends cls_data {
		var $table_reply = '`chh_ksguestbook_reply`';
...
//			}
		...
		function add_hit($id=0){
			if(!isset($_COOKIE['ksguestbook_hit_'.$id])){
				$info =  $this->get_info($id);
				$feild = array();
				$feild['id'] = $info['id'];
				...
				setcookie('ksguestbook_hit_'.$id, 1, $t);	
			}
		}
...
//				$sql =	"LIMIT " . $filter['start'] . ", ".$filter['page_size']." ;";
//			}
			$sql =	"SELECT *, (SELECT COUNT(*) FROM " .$this->table_reply. " WHERE ksguestbook_id=t.id) AS reply_count, (SELECT add_time FROM " .$this->table_reply. "_reply WHERE ksguestbook_id=t.id ORDER BY sort DESC LIMIT 1 ) AS last_reply_time ".
