<?php
	...
	class cls_goods extends cls_data {
		...
		function get_list(){
			...
			foreach($arr['list'] as $k=>$v){
				/* 檢查是否有加入購物車功能 */
				$v['is_add_cart'] = $this->check_add_cat($v);
				$arr['list'][$k] = $v;				
		...
		function upd_sn($id){
			...
			$sn = $GLOBALS['_CFG']['sn_prefix'] . str_repeat('0', 6 - strlen($id)) . $id;
		...
		/* 檢查是否有加入購物車功能 */
		function check_add_cat($info){
			$b = false;
			/* 取得庫存 */
			$is_inventory = ($info['inventory'] > 0) ? true : false;
			/* 價格判斷 */
			$is_price = ($info['price'] > 0) ? true : false;
			/* 判斷 */
			if(HAS_CART && $is_inventory > 0 && $is_price){
				$b = true;
			}
			return $b;
		}
