<?php
	...
	class cls_mlguestbook extends cls_data {
		var $table_reply = '`chh_mlguestbook_reply`';
...
//			}
		...
		function add_hit($id=0){
			if(!isset($_COOKIE['mlguestbook_hit_'.$id])){
				$info =  $this->get_info($id);
				$feild = array();
				$feild['id'] = $info['id'];
				...
				setcookie('mlguestbook_hit_'.$id, 1, $t);	
			}
		}
...
//				$sql =	"LIMIT " . $filter['start'] . ", ".$filter['page_size']." ;";
//			}
			$sql =	"SELECT *, (SELECT COUNT(*) FROM " .$this->table_reply. " WHERE mlguestbook_id=t.id) AS reply_count, (SELECT add_time FROM " .$this->table_reply. "_reply WHERE mlguestbook_id=t.id ORDER BY sort DESC LIMIT 1 ) AS last_reply_time ".
