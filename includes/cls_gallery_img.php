<?php
	...
	class cls_gallery_img extends cls_data {
		
		var $foreign_name	= 'gallery_id';
		var $upload_folder	= 'gallery';	
		var $img_width		= 545;
		var $img_height		= 400;		
		var $thumb_width	= 50;
		var $thumb_height	= 50;
		...
		function upload_img($file, $id = 0){
			...
			if (isset($file['gallery_img']['error'])) // php 4.2 版本才支持 error
			{
				// 最大上傳文件大小
				...
				// 圖片
				if ($file['gallery_img']['error'] == 1)
				{
					make_json_error(sprintf($_LANG['img_too_big'], $php_maxsize));
				}
				elseif ($file['gallery_img']['error'] == 2)
				{
			...
			// 如果上傳圖片，相應處理
			if ($file['gallery_img']['tmp_name'] != '' && $file['gallery_img']['tmp_name'] != 'none')
			{
				if ($id > 0)
				{
					/* 刪除原來的圖片文件 */
				...
				$original_img   = $image->upload_image($file['gallery_img'], $this->upload_folder); // 原始圖片
				...
				/* 依副檔名設定檔案形態 */			
				$suffix = get_file_suffix($file['gallery_img']['name']);
				switch($suffix){
					case 'gif':
						$file['gallery_img']['type'] = 'image/gif';
						break;
					case 'png':
						$file['gallery_img']['type'] = 'image/png';
						break;
					default:
						$file['gallery_img']['type'] = 'image/jpeg';
						break;
				}
				
				// 如果系統支持GD，縮放圖片，且給圖片和相冊圖片加水印			
				if ($image->gd_version() > 0 && $image->check_img_function($file['gallery_img']['type']))
				{
					// 如果設置大小不為0，縮放圖片
